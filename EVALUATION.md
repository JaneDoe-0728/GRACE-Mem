...
```bash
uv run python experiment/benchmarking/evaluation/judge.py --help
```
...
```bash
uv run python experiment/benchmarking/evaluation/judge.py locomo <run-tag> \
  --samples 0-9 \
  --workers 8
```
...
```bash
uv run python experiment/benchmarking/evaluation/judge.py longmem <run-tag> \
  --workers 8
```
...
```bash
uv run python experiment/benchmarking/evaluation/score.py <run-tag>
```

Multiple runs produce an overall mean and population standard deviation:

```bash
uv run python experiment/benchmarking/evaluation/score.py <run-r1> <run-r2> <run-r3>
```
...
uv run python experiment/benchmarking/evaluation/oracle.py longmem oracle-longmem
```
...
run the standardized judge immediately, or run `experiment/benchmarking/evaluation/judge.py` separately.
