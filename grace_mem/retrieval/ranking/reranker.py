"""The pointwise reranker: is this candidate relevant to this query, yes or no.

Runs Qwen3-Reranker-0.6B in generative mode, judging relevance by the yes/no
logit gap under Qwen3 chat format with memory-QA specific instructions per
document type. Two backends:
...
This is the scoring backend only. What is done with a score -- the threshold,
the top-K cut, which candidates are even offered -- is `filter.py`, its one
caller inside retrieval.

Two paths here are found by counting directories up from __file__, and neither
raises when the count is wrong: load_dotenv would silently find no .env (so
RERANKER_API reads as unset and every run falls back to the local backend), and
MODELS_DIR would silently miss the bundled weights.
"""
...
# Repo root: this module is at grace_mem/retrieval/ranking/, so four levels up.
...
class LLMPointwiseReranker:
    ...
    MODELS_DIR = Path(__file__).parent.parent.parent.parent / "models" / "reranker"
