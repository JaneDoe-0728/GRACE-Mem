"""Context filtering: the intersection, and the cut.

`EvidenceFilter` takes the candidate entities and relationships the search and
graph-expansion stages produced, computes the subgraph intersection, scores what
survives with the pointwise reranker, and keeps top-K above a threshold. The
reranker is the filter here -- there is no separate similarity cut in front of
it.

The scoring backend itself is `reranker.py`; this module owns only the policy
applied to its scores.
"""
...
from grace_mem.retrieval.ranking.reranker import get_reranker
