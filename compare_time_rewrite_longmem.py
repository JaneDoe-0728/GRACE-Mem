"""
compare_time_rewrite_longmem.py — LongMem 版：比較兩個 run（baseline vs time-rewrite），
量化 time-rewrite 的影響落在「檢索(gold recall)」還是「答題」。

輸入：兩個 run 的路徑（output/ 下的資料夾名，或絕對路徑）。
    argv1 = baseline run（無 time-rewrite，例如 rerank16-rr2）
    argv2 = tr run      （有 time-rewrite，例如 rerank16-20b63-tr）

用法：
    python compare_time_rewrite_longmem.py <baseline_run> <tr_run> \
        [--out-json PATH] [--n-examples 10] [--max-context-chars 0]
...
重用 experiment/longmem/gold_recall_metrics.py 的 gold-sid / retrieved-sid 計算，
與既有 LongMem gold recall 口徑一致（:u/:a split 粒度、Evidence Summary 區塊）。

LongMem 與 LoCoMo 的差異：一題一個 CSV（<run>/<category>/<id>.csv, 取 df.iloc[0]），
gold 來自 script_data/<category>/<id>.csv 的 has_answer 轉 :u/:a sid。
"""
...
_OUTPUT_ROOT = _ROOT / "experiment" / "longmem" / "output"

# 動態載入 LongMem 的 gold_recall_metrics（避免與根目錄同名檔混淆）
_spec = importlib.util.spec_from_file_location(
    "lm_gold_recall_metrics", _ROOT / "experiment" / "longmem" / "gold_recall_metrics.py"
)
lm = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(lm)  # 提供 _is_main_csv / _retrieved_sids / _gold_sids / CATEGORIES / DATA_ROOT


JACCARD_DOC = (
    "Jaccard(A,B) = |A∩B| / |A∪B|，A/B 是兩個 run 對同一題檢索到的 summary 片段 "
    "(sid，:u/:a 粒度) 集合。1.0 = 兩邊檢索到完全相同的證據；越低代表檢索內容差異越大。"
...
def score_run(run_dir: Path) -> dict:
    ...
    for cat in lm.CATEGORIES:
        ...
        for p in sorted(cdir.glob("*.csv")):
            if not lm._is_main_csv(p):
                continue
            df = pd.read_csv(p)
            if "Retrieved_Context" not in df.columns or len(df) == 0:
                continue
            row = df.iloc[0]  # 一題一個 CSV
            try:
                correct = int(float(str(row.get("correctness", "")).strip())) == 1
            except ValueError:
                correct = False

            # gold sid 來自 dataset（兩個 run 相同）
            src = lm.DATA_ROOT / cat / f"{p.stem}.csv"
            gold = lm._gold_sids(src) if src.exists() else set()
            retrieved = lm._retrieved_sids(str(row.get("Retrieved_Context") or ""))

            recs[(cat, p.stem)] = dict(
                category=cat,
                stem=p.stem,
                ...
                model_answer=str(row.get("Generated_Answer") or ""),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Compare baseline vs time-rewrite LongMem runs")
    ...
    base_recs = score_run(base_dir)
    tr_recs = score_run(tr_dir)

    base_ov, base_pc = aggregate(base_recs)
    tr_ov, tr_pc = aggregate(tr_recs)

    print("=" * 74)
    print(f"baseline = {base_dir.name}   |   time-rewrite = {tr_dir.name}")
    print(f"題數: baseline={len(base_recs)}  tr={len(tr_recs)}  共同題={len(set(base_recs) & set(tr_recs))}")
    print("=" * 74)
    ...
    print("\n  解讀：Gold返回率 vs gold全中的正確率 哪個變動大，就代表影響在檢索端還是答題端。")
    ...
    payload = dict(
        _meta=dict(
            ...
            field_notes={
                "baseline_retrieved_context": "rewrite 前(baseline)給 LLM 的檢索文字(Retrieved_Context)",
                "tr_retrieved_context": "rewrite 後(time-rewrite)給 LLM 的檢索文字",
                "*_retrieved_sids": "各自檢索到的 summary 片段 id（:u/:a 粒度）",
