"""
Re-answer all CSVs in multi_dataset_output using replay_summary_fact_from_run logic:
  1. Load the evidence IDs selected by the prior run (from kg_retrieval_evidence.jsonl)
  2. Parse session_id from each summary_id, reconstruct raw conversation from source CSV
  3. Run fact extraction on the raw conversation text
  4. Re-answer the question with extracted facts as context
  5. Optionally re-judge

Source CSVs are read from: experiment/longmem/script_data/{category}/{name}.csv
...
_ALL_ROLES_FACT_PROMPT = """\
You are a precise fact extractor. Given a text chunk from a conversation or session, extract all facts that are worth remembering long-term.
...
def _load_summaries_meta(artifacts_dir: Path) -> dict[str, dict]:
    ...
    Used only for datetime lookup — raw text comes from source CSV."""
...
def _build_session_raw_texts(source_csv: Path, *, source_roles: str = "all") -> dict[str, str]:
    """Return session text using either all turns or only user turns."""
    ...
    for sid, grp in df.groupby(sid_col, sort=False):
        ...
        for _, row in grp.iterrows():
            ...
            if source_roles == "user" and role_col and role.lower() != "user":
                continue
            content = str(row.get(content_col, "")).strip()
            if not content or content == "nan":
                continue
            prefix = "User: " if source_roles == "user" else (f"{role.capitalize()}: " if role else "")
...
def _extract_facts(
    llm: LLMClient,
    raw_text: str,
    dialogue_datetime: str | None,
    *,
    source_roles: str = "all",
) -> list[str]:
    """Run fact extraction on raw conversation text. Falls back to [raw_text] on error."""
    ...
    try:
        resp = llm.chat(
            messages=[
                {
                    "role": "system",
                    "content": _USER_ONLY_FACT_PROMPT if source_roles == "user" else _ALL_ROLES_FACT_PROMPT,
                },
        ...
        try:
            ...
            result = json.loads(_re.sub(r"[\x00-\x1f\x7f]", " ", json_text))

        def as_text(value) -> str:
            if source_roles == "user" and isinstance(value, list):
        ...
        for f in facts_raw:
            if not isinstance(f, dict):
                continue
            what = as_text(f.get("what")).strip()
            if not what:
                continue
            parts = [what]
            when = as_text(f.get("when")).strip()
            if when and when.upper() != "N/A":
                parts.append(f"When: {when}")
            who = as_text(f.get("who")).strip()
            if who and who.upper() != "N/A":
                parts.append(f"Involving: {who}")
            why = as_text(f.get("why")).strip()
        ...
        if source_roles == "user":
            lines = [line.removeprefix("User:").strip() for line in raw_text.splitlines() if line.strip()]
...
def _build_context(
    ...
    *,
    ...
    """Build fact-extracted context from raw conversation text of selected sessions."""
    lines: list[str] = []
    ...
    for sid in selected_ids:
        ...
        # Get datetime from summaries_meta for context
        meta = summaries_meta.get(sid, {})
        dt = meta.get("dialogue_datetime")

        facts = _extract_facts(llm, raw_text, dt, source_roles=source_roles)
...
def preview_facts_csv(
    ...
    *,
    ...
    """Run only fact extraction and save preview CSV. Returns (processed, skipped)."""
    name = csv_path.stem
    out_path = cat_dir / f"{name}_facts_preview.csv"

    if out_path.exists() and not force:
        print(f"  [SKIP] {name}: preview already exists ({out_path.name})")
        return 0, 1
    ...
    if not logs_dir.exists() or not artifacts_dir.exists() or not source_csv.exists():
        print(f"  [SKIP] {name}: missing logs, artifacts, or source CSV")
        return 0, 1
    ...
    for _, row in df.iterrows():
        ...
        for sid in selected_ids:
            ...
            print(f"  [{session_id}] {len(facts)} facts ({elapsed}s)")

            rows.append({
                "question": question,
                "session_id": session_id,
                "summary_id": sid,
                "dialogue_datetime": dt or "",
                "facts": facts_str,
                "fact_count": len(facts),
                "is_fallback": facts == [raw_text],
            })
...
def process_csv(
    ...
    *,
    ...
    name = csv_path.stem
    suffix = "_replay_fact_user_only.csv" if source_roles == "user" else "_replay_fact.csv"
    out_path = cat_dir / f"{name}{suffix}"
    ...
    evidence_map = _load_evidence_map(logs_dir)
    summaries_meta = _load_summaries_meta(artifacts_dir)
    session_raw_texts = _build_session_raw_texts(source_csv, source_roles=source_roles)

    if not question_map or not evidence_map:
        print(f"  [SKIP] {name}: no retrieval logs found")
        return 0, 1

    # Build question → evidence_ids mapping
    q_to_evidence: dict[str, list[str]] = {}
    for req_id, q in question_map.items():
        if req_id in evidence_map:
            q_to_evidence[q.strip().lower()] = evidence_map[req_id]
...
def main(argv: list[str] | None = None, *, default_source_roles: str = "all") -> None:
    description = "Replay fact extraction on prior LongMem output"
    if default_source_roles == "user":
        description = "Replay user-only fact extraction on prior LongMem output"
    ...
    if default_source_roles == "all":
        parser.add_argument(
            "--facts-only",
            action="store_true",
            help="Only run fact extraction, save {name}_facts_preview.csv",
        )
    else:
        parser.set_defaults(facts_only=False)
    parser.add_argument("--source-roles", choices=("all", "user"), default=default_source_roles)
    parser.add_argument("--force", action="store_true", help="Overwrite existing output files")
    args = parser.parse_args(argv)
    if args.facts_only and args.source_roles != "all":
        parser.error("--facts-only is available only with --source-roles all")
    ...
    for dir_name, category in _DIR_TO_CATEGORY.items():
        ...
        csvs = sorted(
            p
            for p in cat_dir.glob("*.csv")
            if not p.stem.endswith("_abs")
            and not p.stem.endswith("_replay_fact")
            and (args.source_roles == "all" or not p.stem.endswith("_replay_fact_user_only"))
        )
        if args.name:
