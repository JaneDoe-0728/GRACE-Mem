...
與舊 gold_summary_eval.py 的差異:
  - 餵 raw turn text(非 LLMlingua 壓縮、非 user-side-only)→ 不需要 artifacts
  - question / answer / question_date 直接取自 script_data → 不需要既有 run
  - 輸出標準 run 目錄格式 → gold_recall_metrics.py / rejudge 工具直接可用

Usage:
    python -m experiment.longmem.agent_filter.oracle_eval --run-tag oracle-raw
    python -m experiment.longmem.agent_filter.oracle_eval --run-tag oracle-raw --category single_session_user --limit 5
    python -m experiment.longmem.agent_filter.oracle_eval --run-tag oracle-raw --workers 4
"""
