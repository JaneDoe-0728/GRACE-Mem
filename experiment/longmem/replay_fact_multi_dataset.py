...
def _build_session_raw_texts(source_csv: Path) -> dict[str, str]:
    """Returns {session_id: raw_conversation_text} from the source dataset CSV."""
    ...
    for sid, grp in df.groupby(sid_col, sort=False):
        ...
        for _, row in grp.iterrows():
            ...
            if not content or content == "nan":
                continue
            prefix = f"{role.capitalize()}: " if role else ""
...
def process_csv(
    ...
    *,
    ...
    name = csv_path.stem
    out_path = cat_dir / f"{name}_replay_fact.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Replay fact extraction on prior longmem run output")
    ...
    parser.add_argument("--facts-only", action="store_true", help="Only run fact extraction, save {name}_facts_preview.csv")
    ...
    for dir_name, category in _DIR_TO_CATEGORY.items():
        ...
        csvs = sorted(
            ...
            if not p.stem.endswith("_abs") and not p.stem.endswith("_replay_fact")
        )
        if args.name:
